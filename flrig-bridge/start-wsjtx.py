...
"""
Start WSJT-X
Launch WSJT-X with optional FlRig and bridge support

WSJT-X can connect to FlRig directly or use the rigctld bridge
"""
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Start WSJT-X with optional FlRig and bridge',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  %(prog)s                              # Launch WSJT-X only
...
Note: WSJT-X can use FlRig directly (recommended) or connect to rigctld bridge
      Configure rig control in WSJT-X settings
        """
    )
    ...
    parser.add_argument(
        '--start-flrig',
        action='store_true',
        help='Start FlRig before launching WSJT-X'
    )
    parser.add_argument(
        '--config',
        default='g90-sdr',
    ...
    if not check_command_exists('wsjtx'):
        print("       WSJT-X not found", file=sys.stderr)
        print("       Install from: https://physics.princeton.edu/pulsar/k1jt/wsjtx.html", file=sys.stderr)
        return 1
    ...
    try:
        ...
        # Launch WSJT-X
        print("Launching WSJT-X...")
        print("=" * 60)
        print()
        print("Configure rig control in WSJT-X:")
        ...
        wsjtx_process = start_process('wsjtx', [], "WSJT-X")
        if not wsjtx_process:
            raise RuntimeError("Failed to start WSJT-X")

        print("WSJT-X started successfully")
        if processes:
            print("\nBackground processes running:")
            for name in processes.keys():
                print(f"  - {name}")
            print("\nNote: Close WSJT-X to stop this script")

        # Wait for WSJT-X to exit
        wsjtx_process.wait()
        print("\nWJST-X exited")
